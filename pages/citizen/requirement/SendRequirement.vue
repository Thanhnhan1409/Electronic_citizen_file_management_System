    ...
    <div class="list--content-notification">
      ...
      <ul class="responsive-table content">
        <li class="table-header">
          ...
          <div class="col col-1">Mã yêu cầu</div>
          <div class="col col-2">Ngày gửi</div>
          <div class="col col-3">Nội dung</div>
          <div class="col col-4">Trạng thái</div>
         
        </li>
        <li class="table-row display" v-for="(item, index) of listRequirement" :key="item.id_requirement">
          <div class="col col-0" data-label="STT">{{ index + 1 }}</div>
          <div class="col col-1" data-label="Số CCCD">{{ item.id_requirement }}</div>
          <div class="col col-2" data-label="Ngày gửi">{{ item.date }}</div>
          <div class="col col-3" data-label="Nội dung">{{ item.content }}</div>
          <div class="col col-4" data-label="Trạng thái">{{ item.status }}</div>
        </li>
      </ul>
    </div>
            ...
            <div class="popup-content">
              ...
              <ul class="popup-input">
                <li class="popup-input--title">
                 <span>Người nhận yêu cầu: </span>
                  <select>
                <option v-for="(recipient_id, index) in recipient_id" :key="index" :value="recipient_id.value">{{ recipient_id.name }}</option>
                 </select>
                ...
                <button @click.prevent="postData" class="popup-button">Gửi yêu cầu</button>
              </ul>
            </div>
          </div>
  </div>
...
export default {
  ...
  methods: {
   ...
   async asyncData() {
      const response = await axios.get('http://localhost:8080/api/politician/listPolitician/country');
    ...
    async postData() {
         try {
        const res = await this.$axios.post(`http://localhost:8080/api/requirement/new`, {
