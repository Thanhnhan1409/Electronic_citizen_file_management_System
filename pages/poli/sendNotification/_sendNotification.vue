<template>
    <div class="container">
      <div @click.prevent="displayPopup" class="add-notification">
        <svg class="icon-plus" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M240 80c0-17.7-14.3-32-32-32s-32 14.3-32 32V224H32c-17.7 0-32 14.3-32 32s14.3 32 32 32H176V432c0 17.7 14.3 32 32 32s32-14.3 32-32V288H384c17.7 0 32-14.3 32-32s-14.3-32-32-32H240V80z"/></svg>
        <p>Thêm thông báo</p> 
      </div>
      <div class="list--content-notification">
          <h2>Danh dách nội dung thông báo</h2>
            <ul class="responsive-table">
                ...
                <ul class="responsive-table content" v-for="item of listNotification" :key="item.id">
                    <li class="table-row display">
                            ...
                            <div class="col col-1" data-label="Số CCCD Người nhận">
                              <ul class="list-idCitizen--items" v-for="tmp of item.citizens">
                                <li class="item-idCitizen">{{ tmp.name }}</li> 
                              </ul>
                            </div>
                            <div class="col col-2" data-label="Ngày">{{ curDate }}</div>
                            <div class="col col-3" data-label="Nội dung">{{ item.message }}</div>
                    </li>
                </ul>
            </ul>
        </div>
        ...
        <div class="add-notificaito--popup">
          <svg @click.prevent="hiddenPopup" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><!--! Font Awesome Pro 6.3.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path d="M310.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L160 210.7 54.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L114.7 256 9.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L160 301.3 265.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L205.3 256 310.6 150.6z"/></svg>
          <div class="popup-content">
            <h3>Thêm thông báo</h3>
            <ul class="popup-input">
              <li class="popup-input--title">
                <span>Người nhận: </span> 
                <input v-model="stringIdCitizen" type="text" placeholder="Nhập Số CCCD">
              </li>
              <li class="popup-input--title">
                <span>Nội dung:</span>
                <textarea v-model="description" name="" id="" cols="30" rows="10"></textarea>
              </li>
              <button @click.prevent="postData" class="popup-button">Thêm thông báo</button>
            </ul>
          </div>
        </div>
    </div>
...
export default{
  ...
  methods:{
    async postData(){
      ...
      console.log("aaaNhan");
      await this.$axios.post(`http://localhost:8080/api/notification/newNotification`,{
    ...
    async fetchListNotification(){
      try {
        await this.$axios.get(`http://localhost:8080/api/notification/politicianId=${this.author_id}`)
        .then((res)=>{
    ...
    getDate(){
     ...
     this.curDate = currentDate.getDate()+'-'+(currentDate.getMonth()+1)+'-'+currentDate.getFullYear();
    }
  }
}
</script>
