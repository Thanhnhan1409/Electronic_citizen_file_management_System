    ...
    <div class="container-listInfor">
      ...
      <ul class="responsive-table">
        <li class="table-header">
          ...
          <div class="col col-3">{{ isPoli?'Cấp vụ' :"Ngày" }}</div>
          <div class="col col-4">{{ isPoli?'Khu vực' :"Nội dung" }}</div>
          <div class="col col-5">{{ isPoli?'Chức vụ' :"Trạng thái" }}</div>
        </li>
        <ul
        ...
        >
          <li class="table-row display">
            ...
            <div class="col col-1" data-label="Số CCCD công dân">
              {{ isPoli? item.citizen.citizenId : item.author.citizenId }}
            </div>
            <div class="col col-2" data-label="Tên">
              {{ isPoli? item.citizen.name : item.author.name }}
            </div>
            <div class="col col-3" data-label="Ngày">
              {{ isPoli? item.date: item.author }}
            </div>
            <div class="col col-4" data-label="Nội dung">
                {{ isPoli? item.date: item.author }}
            </div>
            <div class="col col-5" data-label="Trạng thái">
              <span class="status-waiting"> {{ isPoli? item.date: item.author }}</span>
            </div>
          </li>
        </ul>
      </ul>
