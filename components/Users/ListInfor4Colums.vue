    ...
    <div class="list-appointment">
      <ul class="responsive-table">
        <li class="table-header">
          ...
          <div class="col col-1">{{isNoti? 'ID CBCC' : 'Tên'}}</div>
          <div class="col col-2" v-if="isNoti">Tên</div>
        ...
        >
          <li class="table-row display">
            ...
            <div  class="col col-1" data-label="Số CCCD công dân">
              {{ isNoti ? item.politician.politicianId : item.citizen.name }}
            </div>
            <div v-if="isNoti" class="col col-2" data-label="Tên">
              {{ isNoti ? item.politician.citizen.name : item.author.name }}
            </div>
            <div class="col col-3" data-label="Ngày">
              {{ isNoti ? item.message : item.content }}
            </div>
          </li>
        </ul>
      </ul>
